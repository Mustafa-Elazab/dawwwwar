...
export function PhoneScreen() {
  ...
  return (
    <ScreenTemplate backgroundColor={colors.primary}>
      <StatusBar barStyle="light-content" backgroundColor={colors.primary} />
      
      <View style={styles.container}>
        {/* Top 30% Illustration/Branding area */}
        <View style={styles.illustrationArea}>
          <Text style={styles.logoText}>{'دوّار'}</Text>
          <Text style={styles.tagline}>{t('home.location_sub')}</Text>
        </View>

        {/* Form Card (Bottom Sheet style) */}
        <View style={styles.card}>
          {/* Form title */}
          <Text style={styles.formTitle}>{t('merchant.auth.phone.title')}</Text>
          ...
          <View style={[styles.phoneRow, ctrl.phoneError ? styles.phoneRowError : null]}>
            ...
            <RTLTextInput
              ...
              placeholder={t('merchant.auth.phone.placeholder')}
          ...
          {/* Send OTP button */}
          <Button
            label={ctrl.isLoading ? t('common.loading') : t('merchant.auth.phone.continue')}
