...
export const DawwarEvents = {
  ...
  otpSent: (phone: string) =>
    logEvent('otp_sent', { phone_prefix: phone.slice(0, 5) }),
  ...
  orderCancelled: (orderId: string) =>
    logEvent('order_cancelled', { order_id: orderId }),
  ...
  favoriteAdded: (merchantId: string) =>
    logEvent('favorite_added', { merchant_id: merchantId }),
  promoCodeApplied: (code: string) =>
    logEvent('promo_code_applied', { code }),

  // Merchant
  orderAccepted: (orderId: string) =>
    logEvent('order_accepted_merchant', { order_id: orderId }),
  orderRejected: (orderId: string) =>
    logEvent('order_rejected_merchant', { order_id: orderId }),
  orderMarkedReady: (orderId: string) =>
    logEvent('order_marked_ready', { order_id: orderId }),
  ...
  productCreated: (productId: string) =>
    logEvent('product_created', { product_id: productId }),

  // Driver
  driverWentOnline: () => logEvent('driver_went_online'),
  driverWentOffline: () => logEvent('driver_went_offline'),
  deliveryAccepted: (orderId: string) =>
    logEvent('delivery_accepted', { order_id: orderId }),
  deliveryDeclined: (orderId: string) =>
    logEvent('delivery_declined', { order_id: orderId }),
  ...
  deliveryCompleted: (orderId: string) =>
    logEvent('delivery_completed', { order_id: orderId }),

  // Wallet
  walletRecharged: (amount: number) =>
    logEvent('wallet_recharged', { amount }),
