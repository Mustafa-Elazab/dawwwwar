...
export function useController() {
  ...
  const selectRoleMutation = useMutation({
    ...
    onSuccess: ({ role }) => {
      ...
      if (role !== 'CUSTOMER') {
        // Merchant/Driver go to pending screen
        navigation.navigate(AUTH_ROUTES.PENDING_APPROVAL);
      }
      // CUSTOMER: RootNavigator detects isAuthenticated + role = CUSTOMER → shows CustomerTabs
    },
  });
